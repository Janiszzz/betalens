...
个券交易状态（停牌、暂停上市等）与行业归属同属 **point-in-time 状态数据**，复用长格式
时序表（``trade_status`` 表）。为节省空间采用 **稀疏存储**：只存「异常状态」与「首次正常
交易日锚点」，绝大多数正常交易日不落库，由查询时推断。

- ``metric``：固定为 ``交易状态``
- ``value``：状态编码。``0`` = 异常（停牌等），``1`` = 正常交易（**仅首次正常交易日**
  入库做锚点）
- ``remark``（JSONB）：``{"status": 状态文本, "first_normal": bool}``，``status`` 存
  原始中文（如 ``停牌一天``/``暂停上市``），``first_normal=true`` 标记首次正常交易日
- ``datetime``：状态生效日（停牌日或首次正常交易日），15:00:01 时点

入库时每只股票只写：所有异常日（``value=0``）+ 首个正常交易日（``value=1``，锚点）。
后续正常日、节假日/周末（原始为 NaN）均不落库。

查询时还原为完整状态
~~~~~~~~~~~~~~~~~~~~

:func:`~betalens.datafeed.query_trade_status` 在 Python 端把稀疏记录还原为每个
``(code, date)`` 的完整状态：
...
.. code-block:: python
   ...
   try:
       # codes=None 表示全市场（取所有有锚点的代码）
       status = ts.query_trade_status({
           "codes": ["000001.SZ"],
           "dates": ["2005-05-30", "2005-06-01", "2005-06-17"],
       })
   finally:
       ts.close()
   # 返回列：code | datetime | value | status_text | name

``value`` 的三态语义：

- ``1`` = 正常交易（首次正常交易日，或其后任一无异常记录的交易日）
- ``0`` = 异常（停牌等，``status_text`` 给出原始中文）
- ``-1`` = 无法交易（**首次正常交易日之前**，视为未上市/未交易）

入库宽表（交易状态快照）
~~~~~~~~~~~~~~~~~~~~~~~~

输入为宽表：第 0 行为简称，第 1 行为 ``Date`` + WindCode，第 2 行起为日期 + 状态文本
（NaN 表示非交易日）。脚本 ``makeupdatabase/load_trade_status.py`` 逐股票列扫描，
生成稀疏长表后经 :func:`~betalens.datafeed.incremental_insert` 去重入库。

.. code-block:: bash

   # 建表（复用通用 DDL 模板）
   python betalens/datafeed/makeupdatabase/create_database.py --tables trade_status

   # 入库
   python betalens/datafeed/makeupdatabase/load_trade_status.py \
       --excel <交易状态快照.xlsx> --sheet Sheet1
