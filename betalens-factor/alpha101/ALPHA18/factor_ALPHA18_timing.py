"""ALPHA18 single-stock timing strategy."""
...
_CONFIG_FILE = _FACTOR_DIR / "factor_ALPHA18_timing.yaml"
...
def compute_alpha18_timing(
    open_wide,
    close_wide,
    *,
    close_open_stddev_window=5,
    close_open_correlation_window=10,
    stock_code=None,
    signal_weight=None,
):
    ...
    return compute_alpha(
        ...
        close_open_stddev_window=close_open_stddev_window,
        close_open_correlation_window=close_open_correlation_window,
    )


def build_spec(config: dict, config_path: str | Path = _CONFIG_FILE) -> FactorSpec:
    options = factor_spec_options(config, config_path)
    parameter_names = set(get_definition(18).parameters)
    formula_kwargs = {
        name: value
        for name, value in options.get("compute_kwargs", {}).items()
        if name in parameter_names
    }
    options["required_history_bars"] = required_history_bars_for_alpha(18, formula_kwargs)
    return FactorSpec(
        name=str(section(config, "meta")["name"]),
        compute=compute_alpha18_timing,
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Run ALPHA18 timing from YAML.")
