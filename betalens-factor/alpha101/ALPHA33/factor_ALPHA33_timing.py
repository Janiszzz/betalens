"""ALPHA33 single-stock timing strategy."""
...
_CONFIG_FILE = _FACTOR_DIR / "factor_ALPHA33_timing.yaml"
...
def compute_alpha33_timing(
    open_wide,
    close_wide,
    *,
    open_close_coefficient=-1,
    open_close_complement_base=1,
    stock_code=None,
    signal_weight=None,
):
    ...
    return compute_alpha(
        ...
        open_close_coefficient=open_close_coefficient,
        open_close_complement_base=open_close_complement_base,
    )


def build_spec(config: dict, config_path: str | Path = _CONFIG_FILE) -> FactorSpec:
    options = factor_spec_options(config, config_path)
    parameter_names = set(get_definition(33).parameters)
    formula_kwargs = {
        name: value
        for name, value in options.get("compute_kwargs", {}).items()
        if name in parameter_names
    }
    options["required_history_bars"] = required_history_bars_for_alpha(33, formula_kwargs)
    return FactorSpec(
        name=str(section(config, "meta")["name"]),
        compute=compute_alpha33_timing,
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Run ALPHA33 timing from YAML.")
