"""ALPHA93 single-stock timing strategy."""
...
_CONFIG_FILE = _FACTOR_DIR / "factor_ALPHA93_timing.yaml"
...
def compute_alpha93_timing(
    ...
    *,
    ...
    indneutralize_vwap_industry_adv_correlation_window=17.4193,
    correlation_indneutralize_vwap_decay_window=19.848,
    decay_linear_correlation_indneutralize_rank_window=7.54455,
    close_mix_weight=0.524434,
    mixed_complement_base=1,
    mixed_complement_weight=0.524434,
    mixed_delta_lag=2.77377,
    delta_mixed_decay_window=16.2664,
    stock_code=None,
    signal_weight=None,
):
    ...
    return compute_alpha(
        ...
        indneutralize_vwap_industry_adv_correlation_window=indneutralize_vwap_industry_adv_correlation_window,
        correlation_indneutralize_vwap_decay_window=correlation_indneutralize_vwap_decay_window,
        decay_linear_correlation_indneutralize_rank_window=decay_linear_correlation_indneutralize_rank_window,
        ...
        mixed_delta_lag=mixed_delta_lag,
        delta_mixed_decay_window=delta_mixed_decay_window,
    )


def build_spec(config: dict, config_path: str | Path = _CONFIG_FILE) -> FactorSpec:
    options = factor_spec_options(config, config_path)
    parameter_names = set(get_definition(93).parameters)
    formula_kwargs = {
        name: value
        for name, value in options.get("compute_kwargs", {}).items()
        if name in parameter_names
    }
    options["required_history_bars"] = required_history_bars_for_alpha(93, formula_kwargs)
    return FactorSpec(
        name=str(section(config, "meta")["name"]),
        compute=compute_alpha93_timing,
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Run ALPHA93 timing from YAML.")
