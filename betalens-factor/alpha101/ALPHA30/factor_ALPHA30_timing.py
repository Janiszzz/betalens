"""ALPHA30 single-stock timing strategy."""
...
_CONFIG_FILE = _FACTOR_DIR / "factor_ALPHA30_timing.yaml"
...
def compute_alpha30_timing(
    ...
    *,
    ...
    return compute_alpha(
        ...
        close_delay_lag_3=close_delay_lag_3,
        close_delay_lag_4=close_delay_lag_4,
        close_delay_lag_5=close_delay_lag_5,
        rank_direction_complement_base=rank_direction_complement_base,
        volume_sum_window=volume_sum_window,
        volume_sum_window_2=volume_sum_window_2,
    )


def build_spec(config: dict, config_path: str | Path = _CONFIG_FILE) -> FactorSpec:
    options = factor_spec_options(config, config_path)
    parameter_names = set(get_definition(30).parameters)
    formula_kwargs = {
        name: value
        for name, value in options.get("compute_kwargs", {}).items()
        if name in parameter_names
    }
    options["required_history_bars"] = required_history_bars_for_alpha(30, formula_kwargs)
    return FactorSpec(
        name=str(section(config, "meta")["name"]),
        compute=compute_alpha30_timing,
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Run ALPHA30 timing from YAML.")
