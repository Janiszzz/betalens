"""ALPHA60 single-stock timing strategy."""
...
_CONFIG_FILE = _FACTOR_DIR / "factor_ALPHA60_timing.yaml"
...
def compute_alpha60_timing(
    close_wide,
    high_wide,
    low_wide,
    volume_wide,
    *,
    scale_rank_oscillator_coefficient=2,
    close_argmax_window=10,
    stock_code=None,
    signal_weight=None,
):
    ...
    return compute_alpha(
        ...
        scale_rank_oscillator_coefficient=scale_rank_oscillator_coefficient,
...
def build_spec(config: dict, config_path: str | Path = _CONFIG_FILE) -> FactorSpec:
    ...
    parameter_names = set(get_definition(60).parameters)
    formula_kwargs = {
        name: value
        for name, value in options.get("compute_kwargs", {}).items()
        if name in parameter_names
    }
    options["required_history_bars"] = required_history_bars_for_alpha(60, formula_kwargs)
    return FactorSpec(
        name=str(section(config, "meta")["name"]),
        compute=compute_alpha60_timing,
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Run ALPHA60 timing from YAML.")
