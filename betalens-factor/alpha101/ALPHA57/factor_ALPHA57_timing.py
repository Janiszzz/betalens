"""ALPHA57 single-stock timing strategy."""
...
_CONFIG_FILE = _FACTOR_DIR / "factor_ALPHA57_timing.yaml"
...
def compute_alpha57_timing(
    close_wide,
    vwap_wide,
    *,
    close_argmax_window=30,
    rank_ts_argmax_close_decay_window=2,
    stock_code=None,
    signal_weight=None,
):
    ...
    return compute_alpha(
        ...
        vwap_wide=vwap_wide,
        close_argmax_window=close_argmax_window,
        rank_ts_argmax_close_decay_window=rank_ts_argmax_close_decay_window,
    )


def build_spec(config: dict, config_path: str | Path = _CONFIG_FILE) -> FactorSpec:
    options = factor_spec_options(config, config_path)
    parameter_names = set(get_definition(57).parameters)
    formula_kwargs = {
        name: value
        for name, value in options.get("compute_kwargs", {}).items()
        if name in parameter_names
    }
    options["required_history_bars"] = required_history_bars_for_alpha(57, formula_kwargs)
    return FactorSpec(
        name=str(section(config, "meta")["name"]),
        compute=compute_alpha57_timing,
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Run ALPHA57 timing from YAML.")
