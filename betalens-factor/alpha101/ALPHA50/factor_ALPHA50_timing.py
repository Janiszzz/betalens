"""ALPHA50 single-stock timing strategy."""
...
_CONFIG_FILE = _FACTOR_DIR / "factor_ALPHA50_timing.yaml"
...
def compute_alpha50_timing(
    volume_wide,
    vwap_wide,
    *,
    rank_volume_rank_vwap_correlation_window=5,
    rank_correlation_volume_maximum_window=5,
    stock_code=None,
    signal_weight=None,
):
    ...
    return compute_alpha(
        ...
        rank_correlation_volume_maximum_window=rank_correlation_volume_maximum_window,
    )


def build_spec(config: dict, config_path: str | Path = _CONFIG_FILE) -> FactorSpec:
    options = factor_spec_options(config, config_path)
    parameter_names = set(get_definition(50).parameters)
    formula_kwargs = {
        name: value
        for name, value in options.get("compute_kwargs", {}).items()
        if name in parameter_names
    }
    options["required_history_bars"] = required_history_bars_for_alpha(50, formula_kwargs)
    return FactorSpec(
        name=str(section(config, "meta")["name"]),
        compute=compute_alpha50_timing,
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Run ALPHA50 timing from YAML.")
