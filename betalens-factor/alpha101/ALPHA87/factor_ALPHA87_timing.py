"""ALPHA87 single-stock timing strategy."""
...
_CONFIG_FILE = _FACTOR_DIR / "factor_ALPHA87_timing.yaml"
...
def compute_alpha87_timing(
    close_wide,
    vwap_wide,
    amount_wide,
    industry_wide,
    *,
    close_mix_weight=0.369701,
    mixed_complement_base=1,
    mixed_complement_weight=0.369701,
    mixed_delta_lag=1.91233,
    delta_mixed_decay_window=2.65461,
    amount_average_window=81,
    indneutralize_industry_adv_close_correlation_window=13.4132,
    corr_decay_window=4.89768,
    decay_linear_corr_rank_window=14.4535,
    stock_code=None,
    signal_weight=None,
):
    ...
    return compute_alpha(
        ...
        indneutralize_industry_adv_close_correlation_window=indneutralize_industry_adv_close_correlation_window,
...
def build_spec(config: dict, config_path: str | Path = _CONFIG_FILE) -> FactorSpec:
    ...
    parameter_names = set(get_definition(87).parameters)
    formula_kwargs = {
        name: value
        for name, value in options.get("compute_kwargs", {}).items()
        if name in parameter_names
    }
    options["required_history_bars"] = required_history_bars_for_alpha(87, formula_kwargs)
    return FactorSpec(
        name=str(section(config, "meta")["name"]),
        compute=compute_alpha87_timing,
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Run ALPHA87 timing from YAML.")
