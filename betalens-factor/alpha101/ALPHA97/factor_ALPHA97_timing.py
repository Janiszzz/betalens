"""ALPHA97 single-stock timing strategy."""
...
_CONFIG_FILE = _FACTOR_DIR / "factor_ALPHA97_timing.yaml"
...
def compute_alpha97_timing(low_wide, vwap_wide, amount_wide, industry_wide, **kwargs):
    return compute_alpha(97, low_wide=low_wide, vwap_wide=vwap_wide, amount_wide=amount_wide, industry_wide=industry_wide, **kwargs)
...
def build_spec(config: dict, config_path: str | Path = _CONFIG_FILE) -> FactorSpec:
    ...
    return FactorSpec(
        ...
        compute=compute_alpha97_timing,
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Run ALPHA97 timing from YAML.")
