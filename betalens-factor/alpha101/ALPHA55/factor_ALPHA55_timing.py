"""ALPHA55 single-stock timing strategy."""
...
_CONFIG_FILE = _FACTOR_DIR / "factor_ALPHA55_timing.yaml"
...
def compute_alpha55_timing(
    close_wide,
    high_wide,
    low_wide,
    volume_wide,
    *,
    low_minimum_window=12,
    high_maximum_window=12,
    low_minimum_window_2=12,
    rank_stochastic_rank_volume_correlation_window=6,
    stock_code=None,
    signal_weight=None,
):
    ...
    return compute_alpha(
        ...
        high_maximum_window=high_maximum_window,
        low_minimum_window_2=low_minimum_window_2,
        rank_stochastic_rank_volume_correlation_window=rank_stochastic_rank_volume_correlation_window,
    )


def build_spec(config: dict, config_path: str | Path = _CONFIG_FILE) -> FactorSpec:
    options = factor_spec_options(config, config_path)
    parameter_names = set(get_definition(55).parameters)
    formula_kwargs = {
        name: value
        for name, value in options.get("compute_kwargs", {}).items()
        if name in parameter_names
    }
    options["required_history_bars"] = required_history_bars_for_alpha(55, formula_kwargs)
    return FactorSpec(
        name=str(section(config, "meta")["name"]),
        compute=compute_alpha55_timing,
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Run ALPHA55 timing from YAML.")
