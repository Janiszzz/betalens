"""ALPHA14 single-stock timing strategy."""
...
_CONFIG_FILE = _FACTOR_DIR / "factor_ALPHA14_timing.yaml"
...
def compute_alpha14_timing(
    open_wide,
    volume_wide,
    returns_wide,
    *,
    returns_delta_lag=3,
    open_volume_correlation_window=10,
    stock_code=None,
    signal_weight=None,
):
    ...
    return compute_alpha(
        ...
        returns_delta_lag=returns_delta_lag,
        open_volume_correlation_window=open_volume_correlation_window,
    )


def build_spec(config: dict, config_path: str | Path = _CONFIG_FILE) -> FactorSpec:
    options = factor_spec_options(config, config_path)
    parameter_names = set(get_definition(14).parameters)
    formula_kwargs = {
        name: value
        for name, value in options.get("compute_kwargs", {}).items()
        if name in parameter_names
    }
    options["required_history_bars"] = required_history_bars_for_alpha(14, formula_kwargs)
    return FactorSpec(
        name=str(section(config, "meta")["name"]),
        compute=compute_alpha14_timing,
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Run ALPHA14 timing from YAML.")
