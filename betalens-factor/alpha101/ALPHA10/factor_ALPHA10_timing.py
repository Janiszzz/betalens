"""ALPHA10 single-stock timing strategy."""
...
_CONFIG_FILE = _FACTOR_DIR / "factor_ALPHA10_timing.yaml"
...
def compute_alpha10_timing(
    close_wide,
    *,
    close_delta_lag=1,
    change_minimum_window=4,
    ts_min_change_threshold=0,
    change_maximum_window=4,
    ts_max_change_threshold=0,
    stock_code=None,
    signal_weight=None,
):
    ...
    return compute_alpha(
        ...
        ts_min_change_threshold=ts_min_change_threshold,
        change_maximum_window=change_maximum_window,
        ts_max_change_threshold=ts_max_change_threshold,
    )


def build_spec(config: dict, config_path: str | Path = _CONFIG_FILE) -> FactorSpec:
    options = factor_spec_options(config, config_path)
    parameter_names = set(get_definition(10).parameters)
    formula_kwargs = {
        name: value
        for name, value in options.get("compute_kwargs", {}).items()
        if name in parameter_names
    }
    options["required_history_bars"] = required_history_bars_for_alpha(10, formula_kwargs)
    return FactorSpec(
        name=str(section(config, "meta")["name"]),
        compute=compute_alpha10_timing,
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Run ALPHA10 timing from YAML.")
