"""ALPHA61 single-stock timing strategy."""
...
_CONFIG_FILE = _FACTOR_DIR / "factor_ALPHA61_timing.yaml"
...
def compute_alpha61_timing(
    vwap_wide,
    amount_wide,
    *,
    vwap_minimum_window=16.1219,
    amount_average_window=180,
    vwap_adv_correlation_window=17.9282,
    stock_code=None,
    signal_weight=None,
):
...
def build_spec(config: dict, config_path: str | Path = _CONFIG_FILE) -> FactorSpec:
    ...
    parameter_names = set(get_definition(61).parameters)
    formula_kwargs = {
        name: value
        for name, value in options.get("compute_kwargs", {}).items()
        if name in parameter_names
    }
    options["required_history_bars"] = required_history_bars_for_alpha(61, formula_kwargs)
    return FactorSpec(
        name=str(section(config, "meta")["name"]),
        compute=compute_alpha61_timing,
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Run ALPHA61 timing from YAML.")
