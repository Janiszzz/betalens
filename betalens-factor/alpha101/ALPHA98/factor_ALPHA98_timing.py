"""ALPHA98 single-stock timing strategy."""
...
_CONFIG_FILE = _FACTOR_DIR / "factor_ALPHA98_timing.yaml"
...
def compute_alpha98_timing(open_wide, vwap_wide, amount_wide, **kwargs):
    return compute_alpha(98, open_wide=open_wide, vwap_wide=vwap_wide, amount_wide=amount_wide, **kwargs)
...
def build_spec(config: dict, config_path: str | Path = _CONFIG_FILE) -> FactorSpec:
    ...
    return FactorSpec(
        ...
        compute=compute_alpha98_timing,
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Run ALPHA98 timing from YAML.")
