"""ALPHA32 single-stock timing strategy."""
...
_CONFIG_FILE = _FACTOR_DIR / "factor_ALPHA32_timing.yaml"
...
def compute_alpha32_timing(
    close_wide,
    vwap_wide,
    *,
    close_mean_window=7,
    scale_correlation_vwap_coefficient=20,
    close_delay_lag=5,
    vwap_delay_close_correlation_window=230,
    stock_code=None,
    signal_weight=None,
):
    ...
    return compute_alpha(
        ...
        scale_correlation_vwap_coefficient=scale_correlation_vwap_coefficient,
        close_delay_lag=close_delay_lag,
        vwap_delay_close_correlation_window=vwap_delay_close_correlation_window,
    )


def build_spec(config: dict, config_path: str | Path = _CONFIG_FILE) -> FactorSpec:
    options = factor_spec_options(config, config_path)
    parameter_names = set(get_definition(32).parameters)
    formula_kwargs = {
        name: value
        for name, value in options.get("compute_kwargs", {}).items()
        if name in parameter_names
    }
    options["required_history_bars"] = required_history_bars_for_alpha(32, formula_kwargs)
    return FactorSpec(
        name=str(section(config, "meta")["name"]),
        compute=compute_alpha32_timing,
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Run ALPHA32 timing from YAML.")
