"""ALPHA65 single-stock timing strategy."""
...
_CONFIG_FILE = _FACTOR_DIR / "factor_ALPHA65_timing.yaml"
...
def compute_alpha65_timing(
    open_wide,
    vwap_wide,
    amount_wide,
    *,
    open_mix_weight=0.00817205,
    mixed_complement_base=1,
    mixed_complement_weight=0.00817205,
    amount_average_window=60,
    adv_sum_window=8.6911,
    mixed_ts_sum_adv_correlation_window=6.40374,
    open_minimum_window=13.635,
    stock_code=None,
    signal_weight=None,
):
...
def build_spec(config: dict, config_path: str | Path = _CONFIG_FILE) -> FactorSpec:
    ...
    parameter_names = set(get_definition(65).parameters)
    formula_kwargs = {
        name: value
        for name, value in options.get("compute_kwargs", {}).items()
        if name in parameter_names
    }
    options["required_history_bars"] = required_history_bars_for_alpha(65, formula_kwargs)
    return FactorSpec(
        name=str(section(config, "meta")["name"]),
        compute=compute_alpha65_timing,
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Run ALPHA65 timing from YAML.")
