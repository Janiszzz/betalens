"""ALPHA68 single-stock timing strategy."""
...
_CONFIG_FILE = _FACTOR_DIR / "factor_ALPHA68_timing.yaml"
...
def compute_alpha68_timing(
    close_wide,
    high_wide,
    low_wide,
    amount_wide,
    *,
    amount_average_window=15,
    rank_high_rank_adv_correlation_window=8.91644,
    correlation_rank_high_rank_window=13.9333,
    close_mix_weight=0.518371,
    mixed_complement_base=1,
    mixed_complement_weight=0.518371,
    mixed_delta_lag=1.06157,
    stock_code=None,
    signal_weight=None,
):
    ...
    return compute_alpha(
        ...
        rank_high_rank_adv_correlation_window=rank_high_rank_adv_correlation_window,
        correlation_rank_high_rank_window=correlation_rank_high_rank_window,
...
def build_spec(config: dict, config_path: str | Path = _CONFIG_FILE) -> FactorSpec:
    ...
    parameter_names = set(get_definition(68).parameters)
    formula_kwargs = {
        name: value
        for name, value in options.get("compute_kwargs", {}).items()
        if name in parameter_names
    }
    options["required_history_bars"] = required_history_bars_for_alpha(68, formula_kwargs)
    return FactorSpec(
        name=str(section(config, "meta")["name"]),
        compute=compute_alpha68_timing,
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Run ALPHA68 timing from YAML.")
