"""ALPHA89 single-stock timing strategy."""
...
_CONFIG_FILE = _FACTOR_DIR / "factor_ALPHA89_timing.yaml"
...
def compute_alpha89_timing(
    low_wide,
    vwap_wide,
    amount_wide,
    industry_wide,
    *,
    amount_average_window=10,
    low_adv_correlation_window=6.94279,
    correlation_low_adv_decay_window=5.51607,
    decay_linear_correlation_low_rank_window=3.79744,
    indneutralize_vwap_industry_delta_lag=3.48158,
    delta_indneutralize_vwap_decay_window=10.1466,
    decay_linear_delta_indneutralize_rank_window=15.3012,
    stock_code=None,
    signal_weight=None,
):
    ...
    return compute_alpha(
        ...
        low_adv_correlation_window=low_adv_correlation_window,
        correlation_low_adv_decay_window=correlation_low_adv_decay_window,
        decay_linear_correlation_low_rank_window=decay_linear_correlation_low_rank_window,
        indneutralize_vwap_industry_delta_lag=indneutralize_vwap_industry_delta_lag,
        delta_indneutralize_vwap_decay_window=delta_indneutralize_vwap_decay_window,
        decay_linear_delta_indneutralize_rank_window=decay_linear_delta_indneutralize_rank_window,
    )


def build_spec(config: dict, config_path: str | Path = _CONFIG_FILE) -> FactorSpec:
    options = factor_spec_options(config, config_path)
    parameter_names = set(get_definition(89).parameters)
    formula_kwargs = {
        name: value
        for name, value in options.get("compute_kwargs", {}).items()
        if name in parameter_names
    }
    options["required_history_bars"] = required_history_bars_for_alpha(89, formula_kwargs)
    return FactorSpec(
        name=str(section(config, "meta")["name"]),
        compute=compute_alpha89_timing,
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Run ALPHA89 timing from YAML.")
