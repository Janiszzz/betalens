"""ALPHA40 single-stock timing strategy."""
...
_CONFIG_FILE = _FACTOR_DIR / "factor_ALPHA40_timing.yaml"
...
def compute_alpha40_timing(
    high_wide,
    volume_wide,
    *,
    high_stddev_window=10,
    high_volume_correlation_window=10,
    stock_code=None,
    signal_weight=None,
):
    ...
    return compute_alpha(
        ...
        high_stddev_window=high_stddev_window,
...
def build_spec(config: dict, config_path: str | Path = _CONFIG_FILE) -> FactorSpec:
    ...
    parameter_names = set(get_definition(40).parameters)
    formula_kwargs = {
        name: value
        for name, value in options.get("compute_kwargs", {}).items()
        if name in parameter_names
    }
    options["required_history_bars"] = required_history_bars_for_alpha(40, formula_kwargs)
    return FactorSpec(
        name=str(section(config, "meta")["name"]),
        compute=compute_alpha40_timing,
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Run ALPHA40 timing from YAML.")
