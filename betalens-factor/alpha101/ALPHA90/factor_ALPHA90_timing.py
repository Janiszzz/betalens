"""ALPHA90 single-stock timing strategy."""
...
_CONFIG_FILE = _FACTOR_DIR / "factor_ALPHA90_timing.yaml"
...
def compute_alpha90_timing(
    close_wide,
    low_wide,
    amount_wide,
    subindustry_wide,
    *,
    close_maximum_window=4.66719,
    amount_average_window=40,
    indneutralize_subindustry_adv_low_correlation_window=5.38375,
    correlation_low_indneutralize_rank_window=3.21856,
    stock_code=None,
    signal_weight=None,
):
    ...
    return compute_alpha(
        ...
        close_maximum_window=close_maximum_window,
        amount_average_window=amount_average_window,
        indneutralize_subindustry_adv_low_correlation_window=indneutralize_subindustry_adv_low_correlation_window,
        correlation_low_indneutralize_rank_window=correlation_low_indneutralize_rank_window,
    )


def build_spec(config: dict, config_path: str | Path = _CONFIG_FILE) -> FactorSpec:
    options = factor_spec_options(config, config_path)
    parameter_names = set(get_definition(90).parameters)
    formula_kwargs = {
        name: value
        for name, value in options.get("compute_kwargs", {}).items()
        if name in parameter_names
    }
    options["required_history_bars"] = required_history_bars_for_alpha(90, formula_kwargs)
    return FactorSpec(
        name=str(section(config, "meta")["name"]),
        compute=compute_alpha90_timing,
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Run ALPHA90 timing from YAML.")
