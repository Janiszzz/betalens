"""ALPHA35 single-stock timing strategy."""
...
_CONFIG_FILE = _FACTOR_DIR / "factor_ALPHA35_timing.yaml"
...
def compute_alpha35_timing(
    ...
    returns_wide,
    *,
    volume_rank_window=32,
    ts_rank_low_close_complement_base=1,
    low_close_high_rank_window=16,
    ts_rank_returns_complement_base=1,
    returns_rank_window=32,
    stock_code=None,
    signal_weight=None,
):
    ...
    return compute_alpha(
        ...
        ts_rank_low_close_complement_base=ts_rank_low_close_complement_base,
        low_close_high_rank_window=low_close_high_rank_window,
        ts_rank_returns_complement_base=ts_rank_returns_complement_base,
        returns_rank_window=returns_rank_window,
    )


def build_spec(config: dict, config_path: str | Path = _CONFIG_FILE) -> FactorSpec:
    options = factor_spec_options(config, config_path)
    parameter_names = set(get_definition(35).parameters)
    formula_kwargs = {
        name: value
        for name, value in options.get("compute_kwargs", {}).items()
        if name in parameter_names
    }
    options["required_history_bars"] = required_history_bars_for_alpha(35, formula_kwargs)
    return FactorSpec(
        name=str(section(config, "meta")["name"]),
        compute=compute_alpha35_timing,
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Run ALPHA35 timing from YAML.")
