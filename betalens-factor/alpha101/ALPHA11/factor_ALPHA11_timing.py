"""ALPHA11 single-stock timing strategy."""
...
_CONFIG_FILE = _FACTOR_DIR / "factor_ALPHA11_timing.yaml"
...
def compute_alpha11_timing(
    close_wide,
    volume_wide,
    vwap_wide,
    *,
    spread_maximum_window=3,
    spread_minimum_window=3,
    volume_delta_lag=3,
    stock_code=None,
    signal_weight=None,
):
    ...
    return compute_alpha(
        ...
        vwap_wide=vwap_wide,
        spread_maximum_window=spread_maximum_window,
        spread_minimum_window=spread_minimum_window,
...
def build_spec(config: dict, config_path: str | Path = _CONFIG_FILE) -> FactorSpec:
    ...
    parameter_names = set(get_definition(11).parameters)
    formula_kwargs = {
        name: value
        for name, value in options.get("compute_kwargs", {}).items()
        if name in parameter_names
    }
    options["required_history_bars"] = required_history_bars_for_alpha(11, formula_kwargs)
    return FactorSpec(
        name=str(section(config, "meta")["name"]),
        compute=compute_alpha11_timing,
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Run ALPHA11 timing from YAML.")
