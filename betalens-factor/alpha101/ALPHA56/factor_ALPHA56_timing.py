"""ALPHA56 single-stock timing strategy."""
...
_CONFIG_FILE = _FACTOR_DIR / "factor_ALPHA56_timing.yaml"
...
def compute_alpha56_timing(
    returns_wide,
    cap_wide,
    *,
    returns_sum_window=10,
    returns_sum_window_2=2,
    ts_sum_returns_sum_window=3,
    stock_code=None,
    signal_weight=None,
):
    ...
    return compute_alpha(
        ...
        ts_sum_returns_sum_window=ts_sum_returns_sum_window,
    )


def build_spec(config: dict, config_path: str | Path = _CONFIG_FILE) -> FactorSpec:
    options = factor_spec_options(config, config_path)
    parameter_names = set(get_definition(56).parameters)
    formula_kwargs = {
        name: value
        for name, value in options.get("compute_kwargs", {}).items()
        if name in parameter_names
    }
    options["required_history_bars"] = required_history_bars_for_alpha(56, formula_kwargs)
    return FactorSpec(
        name=str(section(config, "meta")["name"]),
        compute=compute_alpha56_timing,
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Run ALPHA56 timing from YAML.")
