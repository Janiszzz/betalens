"""ALPHA67 single-stock timing strategy."""
...
_CONFIG_FILE = _FACTOR_DIR / "factor_ALPHA67_timing.yaml"
...
def compute_alpha67_timing(
    ...
    subindustry_wide,
    *,
    high_minimum_window=2.14593,
    amount_average_window=20,
    indneutralize_vwap_sector_indneutralize_subindustry_adv_correlation_wind=6.02936,
    stock_code=None,
    signal_weight=None,
):
    ...
    return compute_alpha(
        ...
        high_minimum_window=high_minimum_window,
        amount_average_window=amount_average_window,
        indneutralize_vwap_sector_indneutralize_subindustry_adv_correlation_wind=indneutralize_vwap_sector_indneutralize_subindustry_adv_correlation_wind,
    )


def build_spec(config: dict, config_path: str | Path = _CONFIG_FILE) -> FactorSpec:
    options = factor_spec_options(config, config_path)
    parameter_names = set(get_definition(67).parameters)
    formula_kwargs = {
        name: value
        for name, value in options.get("compute_kwargs", {}).items()
        if name in parameter_names
    }
    options["required_history_bars"] = required_history_bars_for_alpha(67, formula_kwargs)
    return FactorSpec(
        name=str(section(config, "meta")["name"]),
        compute=compute_alpha67_timing,
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Run ALPHA67 timing from YAML.")
