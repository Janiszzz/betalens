"""ALPHA9 single-stock timing strategy."""
...
_CONFIG_FILE = _FACTOR_DIR / "factor_ALPHA9_timing.yaml"
...
def compute_alpha9_timing(
    close_wide,
    *,
    close_delta_lag=1,
    change_minimum_window=5,
    ts_min_change_threshold=0,
    change_maximum_window=5,
...
def build_spec(config: dict, config_path: str | Path = _CONFIG_FILE) -> FactorSpec:
    ...
    parameter_names = set(get_definition(9).parameters)
    formula_kwargs = {
        name: value
        for name, value in options.get("compute_kwargs", {}).items()
        if name in parameter_names
    }
    options["required_history_bars"] = required_history_bars_for_alpha(9, formula_kwargs)
    return FactorSpec(
        name=str(section(config, "meta")["name"]),
        compute=compute_alpha9_timing,
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Run ALPHA9 timing from YAML.")
