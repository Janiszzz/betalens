"""ALPHA24 single-stock timing strategy."""
...
_CONFIG_FILE = _FACTOR_DIR / "factor_ALPHA24_timing.yaml"
...
def compute_alpha24_timing(
    close_wide,
    *,
    close_mean_window=100,
    ts_mean_close_delta_lag=100,
    close_delay_lag=100,
    trend_threshold=0.05,
    close_minimum_window=100,
    close_delta_lag=3,
    stock_code=None,
    signal_weight=None,
):
    ...
    return compute_alpha(
        ...
        ts_mean_close_delta_lag=ts_mean_close_delta_lag,
        close_delay_lag=close_delay_lag,
        trend_threshold=trend_threshold,
        close_minimum_window=close_minimum_window,
...
def build_spec(config: dict, config_path: str | Path = _CONFIG_FILE) -> FactorSpec:
    ...
    parameter_names = set(get_definition(24).parameters)
    formula_kwargs = {
        name: value
        for name, value in options.get("compute_kwargs", {}).items()
        if name in parameter_names
    }
    options["required_history_bars"] = required_history_bars_for_alpha(24, formula_kwargs)
    return FactorSpec(
        name=str(section(config, "meta")["name"]),
        compute=compute_alpha24_timing,
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Run ALPHA24 timing from YAML.")
