"""ALPHA21 single-stock timing strategy."""
...
_CONFIG_FILE = _FACTOR_DIR / "factor_ALPHA21_timing.yaml"
...
def compute_alpha21_timing(
    close_wide,
    volume_wide,
    amount_wide,
    *,
    close_mean_window=8,
    close_mean_window_2=2,
    close_stddev_window=8,
    close_stddev_window_2=8,
    amount_average_window=20,
    volume_adv_threshold=1,
    condition1_true_value=-1.0,
    condition2_true_value=1.0,
    volume_condition_true_value=1.0,
    volume_condition_false_value=-1.0,
    stock_code=None,
    signal_weight=None,
):
    ...
    return compute_alpha(
        ...
        close_mean_window_2=close_mean_window_2,
        close_stddev_window=close_stddev_window,
        close_stddev_window_2=close_stddev_window_2,
        amount_average_window=amount_average_window,
        volume_adv_threshold=volume_adv_threshold,
        condition1_true_value=condition1_true_value,
        condition2_true_value=condition2_true_value,
        volume_condition_true_value=volume_condition_true_value,
        volume_condition_false_value=volume_condition_false_value,
    )


def build_spec(config: dict, config_path: str | Path = _CONFIG_FILE) -> FactorSpec:
    options = factor_spec_options(config, config_path)
    parameter_names = set(get_definition(21).parameters)
    formula_kwargs = {
        name: value
        for name, value in options.get("compute_kwargs", {}).items()
        if name in parameter_names
    }
    options["required_history_bars"] = required_history_bars_for_alpha(21, formula_kwargs)
    return FactorSpec(
        name=str(section(config, "meta")["name"]),
        compute=compute_alpha21_timing,
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Run ALPHA21 timing from YAML.")
