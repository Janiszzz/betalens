"""ALPHA84 single-stock timing strategy."""
...
_CONFIG_FILE = _FACTOR_DIR / "factor_ALPHA84_timing.yaml"
...
def compute_alpha84_timing(
    close_wide,
    vwap_wide,
    *,
    vwap_maximum_window=15.3217,
    vwap_ts_max_rank_window=20.7127,
    close_delta_lag=4.96796,
    stock_code=None,
    signal_weight=None,
):
    ...
    return compute_alpha(
        ...
        vwap_wide=vwap_wide,
        vwap_maximum_window=vwap_maximum_window,
        vwap_ts_max_rank_window=vwap_ts_max_rank_window,
...
def build_spec(config: dict, config_path: str | Path = _CONFIG_FILE) -> FactorSpec:
    ...
    parameter_names = set(get_definition(84).parameters)
    formula_kwargs = {
        name: value
        for name, value in options.get("compute_kwargs", {}).items()
        if name in parameter_names
    }
    options["required_history_bars"] = required_history_bars_for_alpha(84, formula_kwargs)
    return FactorSpec(
        name=str(section(config, "meta")["name"]),
        compute=compute_alpha84_timing,
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Run ALPHA84 timing from YAML.")
