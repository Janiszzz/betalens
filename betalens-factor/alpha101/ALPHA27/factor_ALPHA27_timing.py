"""ALPHA27 single-stock timing strategy."""
...
_CONFIG_FILE = _FACTOR_DIR / "factor_ALPHA27_timing.yaml"
...
def compute_alpha27_timing(
    volume_wide,
    vwap_wide,
    *,
    rank_volume_rank_vwap_correlation_window=6,
    correlation_rank_volume_sum_window=2,
    ts_sum_correlation_rank_divisor=2.0,
    value_threshold=0.5,
    value_true_value=-1.0,
    value_false_value=1.0,
    stock_code=None,
    signal_weight=None,
):
    ...
    return compute_alpha(
        ...
        vwap_wide=vwap_wide,
        rank_volume_rank_vwap_correlation_window=rank_volume_rank_vwap_correlation_window,
        correlation_rank_volume_sum_window=correlation_rank_volume_sum_window,
        ts_sum_correlation_rank_divisor=ts_sum_correlation_rank_divisor,
        value_threshold=value_threshold,
        value_true_value=value_true_value,
        value_false_value=value_false_value,
    )


def build_spec(config: dict, config_path: str | Path = _CONFIG_FILE) -> FactorSpec:
    options = factor_spec_options(config, config_path)
    parameter_names = set(get_definition(27).parameters)
    formula_kwargs = {
        name: value
        for name, value in options.get("compute_kwargs", {}).items()
        if name in parameter_names
    }
    options["required_history_bars"] = required_history_bars_for_alpha(27, formula_kwargs)
    return FactorSpec(
        name=str(section(config, "meta")["name"]),
        compute=compute_alpha27_timing,
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Run ALPHA27 timing from YAML.")
