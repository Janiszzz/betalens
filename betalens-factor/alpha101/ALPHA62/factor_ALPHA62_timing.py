"""ALPHA62 single-stock timing strategy."""
...
_CONFIG_FILE = _FACTOR_DIR / "factor_ALPHA62_timing.yaml"
...
def compute_alpha62_timing(
    ...
    *,
    ...
    adv_sum_window=22.4101,
    vwap_ts_sum_adv_correlation_window=9.91009,
    high_low_divisor=2,
    stock_code=None,
    signal_weight=None,
):
...
def build_spec(config: dict, config_path: str | Path = _CONFIG_FILE) -> FactorSpec:
    ...
    parameter_names = set(get_definition(62).parameters)
    formula_kwargs = {
        name: value
        for name, value in options.get("compute_kwargs", {}).items()
        if name in parameter_names
    }
    options["required_history_bars"] = required_history_bars_for_alpha(62, formula_kwargs)
    return FactorSpec(
        name=str(section(config, "meta")["name"]),
        compute=compute_alpha62_timing,
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Run ALPHA62 timing from YAML.")
