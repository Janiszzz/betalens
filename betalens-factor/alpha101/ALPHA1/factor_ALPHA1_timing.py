"""ALPHA1 single-stock timing strategy."""
...
_CONFIG_FILE = _FACTOR_DIR / "factor_ALPHA1_timing.yaml"
...
def compute_alpha1_timing(
    close_wide,
    returns_wide,
    *,
    returns_threshold=0,
    returns_stddev_window=20,
    base_power_exponent=2.0,
    signed_power_base_argmax_window=5,
    rank_ts_argmax_signed_power_center=0.5,
    stock_code=None,
    signal_weight=None,
):
    ...
    return compute_alpha(
        ...
        base_power_exponent=base_power_exponent,
        signed_power_base_argmax_window=signed_power_base_argmax_window,
        rank_ts_argmax_signed_power_center=rank_ts_argmax_signed_power_center,
    )


def build_spec(config: dict, config_path: str | Path = _CONFIG_FILE) -> FactorSpec:
    options = factor_spec_options(config, config_path)
    parameter_names = set(get_definition(1).parameters)
    formula_kwargs = {
        name: value
        for name, value in options.get("compute_kwargs", {}).items()
        if name in parameter_names
    }
    options["required_history_bars"] = required_history_bars_for_alpha(1, formula_kwargs)
    return FactorSpec(
        name=str(section(config, "meta")["name"]),
        compute=compute_alpha1_timing,
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Run ALPHA1 timing from YAML.")
