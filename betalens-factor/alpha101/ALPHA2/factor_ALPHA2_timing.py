"""ALPHA2 single-stock timing strategy."""
...
_CONFIG_FILE = _FACTOR_DIR / "factor_ALPHA2_timing.yaml"
...
def compute_alpha2_timing(
    open_wide,
    close_wide,
    volume_wide,
    *,
    volume_delta_lag=2,
    rank_delta_volume_rank_open_close_correlation_window=6,
    stock_code=None,
    signal_weight=None,
):
    ...
    return compute_alpha(
        ...
        volume_delta_lag=volume_delta_lag,
        rank_delta_volume_rank_open_close_correlation_window=rank_delta_volume_rank_open_close_correlation_window,
    )


def build_spec(config: dict, config_path: str | Path = _CONFIG_FILE) -> FactorSpec:
    options = factor_spec_options(config, config_path)
    parameter_names = set(get_definition(2).parameters)
    formula_kwargs = {
        name: value
        for name, value in options.get("compute_kwargs", {}).items()
        if name in parameter_names
    }
    options["required_history_bars"] = required_history_bars_for_alpha(2, formula_kwargs)
    return FactorSpec(
        name=str(section(config, "meta")["name"]),
        compute=compute_alpha2_timing,
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Run ALPHA2 timing from YAML.")
