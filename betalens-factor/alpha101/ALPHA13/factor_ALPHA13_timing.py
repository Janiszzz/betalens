"""ALPHA13 single-stock timing strategy."""
...
_CONFIG_FILE = _FACTOR_DIR / "factor_ALPHA13_timing.yaml"
...
def compute_alpha13_timing(
    close_wide,
    volume_wide,
    *,
    rank_close_rank_volume_covariance_window=5,
    stock_code=None,
    signal_weight=None,
):
    ...
    return compute_alpha(
        ...
        rank_close_rank_volume_covariance_window=rank_close_rank_volume_covariance_window,
    )


def build_spec(config: dict, config_path: str | Path = _CONFIG_FILE) -> FactorSpec:
    options = factor_spec_options(config, config_path)
    parameter_names = set(get_definition(13).parameters)
    formula_kwargs = {
        name: value
        for name, value in options.get("compute_kwargs", {}).items()
        if name in parameter_names
    }
    options["required_history_bars"] = required_history_bars_for_alpha(13, formula_kwargs)
    return FactorSpec(
        name=str(section(config, "meta")["name"]),
        compute=compute_alpha13_timing,
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Run ALPHA13 timing from YAML.")
