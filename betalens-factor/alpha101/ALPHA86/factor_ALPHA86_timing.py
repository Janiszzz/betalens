"""ALPHA86 single-stock timing strategy."""
...
_CONFIG_FILE = _FACTOR_DIR / "factor_ALPHA86_timing.yaml"
...
def compute_alpha86_timing(
    close_wide,
    vwap_wide,
    amount_wide,
    *,
    amount_average_window=20,
    adv_sum_window=14.7444,
    close_ts_sum_adv_correlation_window=6.00049,
    correlation_close_ts_sum_rank_window=20.4195,
    stock_code=None,
    signal_weight=None,
):
    ...
    return compute_alpha(
        ...
        correlation_close_ts_sum_rank_window=correlation_close_ts_sum_rank_window,
    )


def build_spec(config: dict, config_path: str | Path = _CONFIG_FILE) -> FactorSpec:
    options = factor_spec_options(config, config_path)
    parameter_names = set(get_definition(86).parameters)
    formula_kwargs = {
        name: value
        for name, value in options.get("compute_kwargs", {}).items()
        if name in parameter_names
    }
    options["required_history_bars"] = required_history_bars_for_alpha(86, formula_kwargs)
    return FactorSpec(
        name=str(section(config, "meta")["name"]),
        compute=compute_alpha86_timing,
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Run ALPHA86 timing from YAML.")
