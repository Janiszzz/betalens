"""ALPHA52 single-stock timing strategy."""
...
_CONFIG_FILE = _FACTOR_DIR / "factor_ALPHA52_timing.yaml"
...
def compute_alpha52_timing(
    low_wide,
    volume_wide,
    returns_wide,
    *,
    low_minimum_window=5,
    low_minimum_window_2=5,
    ts_min_low_delay_lag=5,
    returns_sum_window=240,
    returns_sum_window_2=20,
    ts_sum_returns_divisor=220,
    ...
    return compute_alpha(
        ...
        low_minimum_window=low_minimum_window,
        low_minimum_window_2=low_minimum_window_2,
        ts_min_low_delay_lag=ts_min_low_delay_lag,
        returns_sum_window=returns_sum_window,
        returns_sum_window_2=returns_sum_window_2,
        ts_sum_returns_divisor=ts_sum_returns_divisor,
...
def build_spec(config: dict, config_path: str | Path = _CONFIG_FILE) -> FactorSpec:
    ...
    parameter_names = set(get_definition(52).parameters)
    formula_kwargs = {
        name: value
        for name, value in options.get("compute_kwargs", {}).items()
        if name in parameter_names
    }
    options["required_history_bars"] = required_history_bars_for_alpha(52, formula_kwargs)
    return FactorSpec(
        name=str(section(config, "meta")["name"]),
        compute=compute_alpha52_timing,
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Run ALPHA52 timing from YAML.")
