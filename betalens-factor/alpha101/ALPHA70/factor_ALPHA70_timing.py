"""ALPHA70 single-stock timing strategy."""
...
_CONFIG_FILE = _FACTOR_DIR / "factor_ALPHA70_timing.yaml"
...
def compute_alpha70_timing(
    close_wide,
    vwap_wide,
    amount_wide,
    industry_wide,
    *,
    vwap_delta_lag=1.29456,
    amount_average_window=50,
    indneutralize_close_industry_adv_correlation_window=17.8256,
    correlation_indneutralize_close_rank_window=17.9171,
    stock_code=None,
    signal_weight=None,
):
    ...
    return compute_alpha(
        ...
        indneutralize_close_industry_adv_correlation_window=indneutralize_close_industry_adv_correlation_window,
        correlation_indneutralize_close_rank_window=correlation_indneutralize_close_rank_window,
    )


def build_spec(config: dict, config_path: str | Path = _CONFIG_FILE) -> FactorSpec:
    options = factor_spec_options(config, config_path)
    parameter_names = set(get_definition(70).parameters)
    formula_kwargs = {
        name: value
        for name, value in options.get("compute_kwargs", {}).items()
        if name in parameter_names
    }
    options["required_history_bars"] = required_history_bars_for_alpha(70, formula_kwargs)
    return FactorSpec(
        name=str(section(config, "meta")["name"]),
        compute=compute_alpha70_timing,
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Run ALPHA70 timing from YAML.")
