"""ALPHA5 single-stock timing strategy."""
...
_CONFIG_FILE = _FACTOR_DIR / "factor_ALPHA5_timing.yaml"
...
def compute_alpha5_timing(
    open_wide,
    close_wide,
    vwap_wide,
    *,
    vwap_sum_window=10,
    ts_sum_vwap_divisor=10,
    stock_code=None,
    signal_weight=None,
):
    ...
    return compute_alpha(
        ...
        vwap_wide=vwap_wide,
        vwap_sum_window=vwap_sum_window,
        ts_sum_vwap_divisor=ts_sum_vwap_divisor,
    )


def build_spec(config: dict, config_path: str | Path = _CONFIG_FILE) -> FactorSpec:
    options = factor_spec_options(config, config_path)
    parameter_names = set(get_definition(5).parameters)
    formula_kwargs = {
        name: value
        for name, value in options.get("compute_kwargs", {}).items()
        if name in parameter_names
    }
    options["required_history_bars"] = required_history_bars_for_alpha(5, formula_kwargs)
    return FactorSpec(
        name=str(section(config, "meta")["name"]),
        compute=compute_alpha5_timing,
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Run ALPHA5 timing from YAML.")
