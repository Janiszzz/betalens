"""ALPHA58 single-stock timing strategy."""
...
_CONFIG_FILE = _FACTOR_DIR / "factor_ALPHA58_timing.yaml"
...
def compute_alpha58_timing(
    volume_wide,
    vwap_wide,
    sector_wide,
    *,
    indneutralize_vwap_sector_volume_correlation_window=3.92795,
    value_decay_window=7.89291,
    decay_linear_value_rank_window=5.50322,
    stock_code=None,
    signal_weight=None,
):
    ...
    return compute_alpha(
        ...
        indneutralize_vwap_sector_volume_correlation_window=indneutralize_vwap_sector_volume_correlation_window,
...
def build_spec(config: dict, config_path: str | Path = _CONFIG_FILE) -> FactorSpec:
    ...
    parameter_names = set(get_definition(58).parameters)
    formula_kwargs = {
        name: value
        for name, value in options.get("compute_kwargs", {}).items()
        if name in parameter_names
    }
    options["required_history_bars"] = required_history_bars_for_alpha(58, formula_kwargs)
    return FactorSpec(
        name=str(section(config, "meta")["name"]),
        compute=compute_alpha58_timing,
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Run ALPHA58 timing from YAML.")
