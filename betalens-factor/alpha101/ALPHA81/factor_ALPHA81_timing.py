"""ALPHA81 single-stock timing strategy."""
...
_CONFIG_FILE = _FACTOR_DIR / "factor_ALPHA81_timing.yaml"
...
def compute_alpha81_timing(
    volume_wide,
    vwap_wide,
    amount_wide,
    *,
    amount_average_window=10,
    adv_sum_window=49.6054,
    vwap_ts_sum_adv_correlation_window=8.47743,
    left_constant=4,
    rank_corr_product_window=14.9655,
    rank_vwap_rank_volume_correlation_window=5.07914,
    stock_code=None,
    signal_weight=None,
):
    ...
    return compute_alpha(
        ...
        rank_corr_product_window=rank_corr_product_window,
...
def build_spec(config: dict, config_path: str | Path = _CONFIG_FILE) -> FactorSpec:
    ...
    parameter_names = set(get_definition(81).parameters)
    formula_kwargs = {
        name: value
        for name, value in options.get("compute_kwargs", {}).items()
        if name in parameter_names
    }
    options["required_history_bars"] = required_history_bars_for_alpha(81, formula_kwargs)
    return FactorSpec(
        name=str(section(config, "meta")["name"]),
        compute=compute_alpha81_timing,
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Run ALPHA81 timing from YAML.")
