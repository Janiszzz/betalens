"""ALPHA12 single-stock timing strategy."""
...
def build_spec(config: dict, config_path: str | Path = _CONFIG_FILE) -> FactorSpec:
    ...
    parameter_names = set(get_definition(12).parameters)
    formula_kwargs = {
        name: value
        for name, value in options.get("compute_kwargs", {}).items()
        if name in parameter_names
    }
    options["required_history_bars"] = required_history_bars_for_alpha(12, formula_kwargs)
    return FactorSpec(
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Run ALPHA12 timing from YAML.")
