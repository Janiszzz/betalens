"""ALPHA72 single-stock timing strategy."""
...
_CONFIG_FILE = _FACTOR_DIR / "factor_ALPHA72_timing.yaml"
...
def compute_alpha72_timing(
    ...
    *,
    ...
    high_low_adv_correlation_window=8.93345,
    correlation_adv_high_decay_window=10.1519,
    vwap_rank_window=3.72469,
    volume_rank_window=18.5188,
    ts_rank_vwap_ts_rank_volume_correlation_window=6.86671,
    correlation_ts_rank_vwap_decay_window=2.95011,
    stock_code=None,
    signal_weight=None,
):
    ...
    return compute_alpha(
        ...
        high_low_adv_correlation_window=high_low_adv_correlation_window,
        correlation_adv_high_decay_window=correlation_adv_high_decay_window,
        vwap_rank_window=vwap_rank_window,
        volume_rank_window=volume_rank_window,
        ts_rank_vwap_ts_rank_volume_correlation_window=ts_rank_vwap_ts_rank_volume_correlation_window,
        correlation_ts_rank_vwap_decay_window=correlation_ts_rank_vwap_decay_window,
    )


def build_spec(config: dict, config_path: str | Path = _CONFIG_FILE) -> FactorSpec:
    options = factor_spec_options(config, config_path)
    parameter_names = set(get_definition(72).parameters)
    formula_kwargs = {
        name: value
        for name, value in options.get("compute_kwargs", {}).items()
        if name in parameter_names
    }
    options["required_history_bars"] = required_history_bars_for_alpha(72, formula_kwargs)
    return FactorSpec(
        name=str(section(config, "meta")["name"]),
        compute=compute_alpha72_timing,
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Run ALPHA72 timing from YAML.")
