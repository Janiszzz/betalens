"""ALPHA37 single-stock timing strategy."""
...
_CONFIG_FILE = _FACTOR_DIR / "factor_ALPHA37_timing.yaml"
...
def compute_alpha37_timing(
    open_wide,
    close_wide,
    *,
    open_close_delay_lag=1,
    delay_open_close_close_correlation_window=200,
    stock_code=None,
    signal_weight=None,
):
    ...
    return compute_alpha(
        ...
        open_close_delay_lag=open_close_delay_lag,
        delay_open_close_close_correlation_window=delay_open_close_close_correlation_window,
    )


def build_spec(config: dict, config_path: str | Path = _CONFIG_FILE) -> FactorSpec:
    options = factor_spec_options(config, config_path)
    parameter_names = set(get_definition(37).parameters)
    formula_kwargs = {
        name: value
        for name, value in options.get("compute_kwargs", {}).items()
        if name in parameter_names
    }
    options["required_history_bars"] = required_history_bars_for_alpha(37, formula_kwargs)
    return FactorSpec(
        name=str(section(config, "meta")["name"]),
        compute=compute_alpha37_timing,
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Run ALPHA37 timing from YAML.")
