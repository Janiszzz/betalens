"""ALPHA15 single-stock timing strategy."""
...
_CONFIG_FILE = _FACTOR_DIR / "factor_ALPHA15_timing.yaml"
...
def compute_alpha15_timing(
    high_wide,
    volume_wide,
    *,
    rank_high_rank_volume_correlation_window=3,
    rank_correlation_high_sum_window=3,
    stock_code=None,
    signal_weight=None,
):
    ...
    return compute_alpha(
        ...
        rank_high_rank_volume_correlation_window=rank_high_rank_volume_correlation_window,
        rank_correlation_high_sum_window=rank_correlation_high_sum_window,
    )


def build_spec(config: dict, config_path: str | Path = _CONFIG_FILE) -> FactorSpec:
    options = factor_spec_options(config, config_path)
    parameter_names = set(get_definition(15).parameters)
    formula_kwargs = {
        name: value
        for name, value in options.get("compute_kwargs", {}).items()
        if name in parameter_names
    }
    options["required_history_bars"] = required_history_bars_for_alpha(15, formula_kwargs)
    return FactorSpec(
        name=str(section(config, "meta")["name"]),
        compute=compute_alpha15_timing,
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Run ALPHA15 timing from YAML.")
