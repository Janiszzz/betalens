"""ALPHA74 single-stock timing strategy."""
...
_CONFIG_FILE = _FACTOR_DIR / "factor_ALPHA74_timing.yaml"
...
def compute_alpha74_timing(
    ...
    *,
    ...
    adv_sum_window=37.4843,
    close_ts_sum_adv_correlation_window=15.1365,
    high_mix_weight=0.0261661,
    mixed_complement_base=1,
    mixed_complement_weight=0.0261661,
    rank_mixed_rank_volume_correlation_window=11.4791,
    stock_code=None,
    signal_weight=None,
):
    ...
    return compute_alpha(
        ...
        close_ts_sum_adv_correlation_window=close_ts_sum_adv_correlation_window,
        ...
        rank_mixed_rank_volume_correlation_window=rank_mixed_rank_volume_correlation_window,
    )


def build_spec(config: dict, config_path: str | Path = _CONFIG_FILE) -> FactorSpec:
    options = factor_spec_options(config, config_path)
    parameter_names = set(get_definition(74).parameters)
    formula_kwargs = {
        name: value
        for name, value in options.get("compute_kwargs", {}).items()
        if name in parameter_names
    }
    options["required_history_bars"] = required_history_bars_for_alpha(74, formula_kwargs)
    return FactorSpec(
        name=str(section(config, "meta")["name"]),
        compute=compute_alpha74_timing,
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Run ALPHA74 timing from YAML.")
