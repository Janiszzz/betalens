"""ALPHA59 single-stock timing strategy."""
...
_CONFIG_FILE = _FACTOR_DIR / "factor_ALPHA59_timing.yaml"
...
def compute_alpha59_timing(volume_wide, vwap_wide, industry_wide, **kwargs):
    return compute_alpha(59, volume_wide=volume_wide, vwap_wide=vwap_wide, industry_wide=industry_wide, **kwargs)
...
def build_spec(config: dict, config_path: str | Path = _CONFIG_FILE) -> FactorSpec:
    ...
    return FactorSpec(
        ...
        compute=compute_alpha59_timing,
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Run ALPHA59 timing from YAML.")
